...
Why things are built the way they are — the reasoning past choices future
maintainers (and agents) shouldn't have to re-derive from scratch. Kept
together on one page given this project's size; if the codebase grows
enough to warrant it, split into `docs/decisions/ADR-NNN-*.md` per the usual
one-file-per-decision convention.
...
**Consequences:** Deployment is a single file plus a migration step — no
...
auth flow couldn't provide. Rutgers SSO integration (SAML2/OIDC) remains a
deliberately deferred future integration, not a bug — there's no SSO code
path to "finish," only one to add if/when it's prioritized.
...
upload — back up `FERNET_KEY` with the same rigor as the database itself.
...
spaCy's model fails to load (regex-only fallback, logged), and is layered
with an architectural guarantee that full spreadsheet rows are never in the
AI request path at all regardless of scrubbing. It is not a substitute for
...
reviewed and commented on by a mentor, never redistributed as a file from
someone else's link. Anyone building a feature that touches mentor sharing
should treat "mentors cannot download" as a firm product boundary, not an
oversight to "fix."
...
**Decision:** Keep the single 12-point threshold. This has been
independently re-verified (and correctly *not* changed) during a prior
documentation/fidelity audit of this codebase, after Q6's UI helper text was
briefly edited to imply a second threshold at 10 before being reverted back
to match the actual algorithm.

**Consequences:** Anyone touching `select_template` or Q6's helper text
(`web/src/screens/IntakeQuestions.jsx`) should treat the single-threshold
behavior as the validated contract, not a simplification to "fix" by adding
a second branch — that would be a real behavior change requiring its own
design sign-off, not a copy edit.
...
test?" question — the app's north star. It also means the exported R/SPSS/
SAS code (Q9) has to encode the *runtime* decision as a comment/branch
