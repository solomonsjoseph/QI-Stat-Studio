...
(your latest intake answers, upload, analysis run, and any edits), so it
never puts you at the wrong step even if you closed the tab mid-workflow.
...
interpretation so the report can't mix data from two different runs.
...
Mentor links expire automatically after 30 days, but you don't have to wait:
revoking immediately invalidates a link (the mentor gets a "not found" page),
and regenerating revokes the old link while issuing a brand-new one — useful
if a link was sent to the wrong address. Both actions are available from the
project's share management; ask your admin if you don't see them exposed in
your build, since some deployments manage this through the API directly
(`POST /share/{project_id}/revoke` / `POST /share/{project_id}/regenerate`
via the API docs at `/docs`).
...
mentor posts them. Mentors can edit or delete their own comments (they need
to re-enter the email they originally commented with — this is what proves
it's the same mentor, since mentors never create an account).

## Update your report before re-downloading

Go back to **Edit & Review**, change the title, caption, or interpretation,
and click **Save & Continue**. Every prior version of an edited field is kept
in the report's Audit Trail — nothing is silently overwritten — so a mentor
or program director can see exactly what you changed and when.

## Archive a project you're done with

From the landing page, click **Archive** next to a project. Archived
projects drop out of your default project list but are not deleted; ask your
admin if you need a project permanently removed.

## Admin: manage runtime settings

Admins see a **Settings** link (not shown to residents). It lists the
current values for things like the clinic name shown in reports, which AI
...
- `POST /notifications/deadline-reminders/run` — sends a reminder email to
  any mentor whose share has a deadline within the next 7 days who hasn't
  already been reminded. Run this periodically (e.g. from a scheduled job)
  if your deployment wants automatic reminders.
